...
**Status:** **S0–S5 shipped**; live helper ingest of nginx ModSec audit; **#640** GET/ingest **starter product rules**; **DL2** widens to **1001–1006** (`1001` xmlrpc, `1002` SQLi ARGS, `1003` URI `../`, `1004` lab, `1005` wp-login POST+payload chain, `1006` URI eval/base64). GET `/wp-admin/` is **not** matched. Imunify/CRS/static IDs are **not** product events. SPA **Simulate** still inserts `mock.sqli.1` (lab-only). **P14 F** adds **protect** on **Host Multi** (customer nginx `SecRuleEngine On`). Product **P12 Host Protect** is on `main` (malware files). This epic is **HTTP request filtering** for the **same Guard-enrolled VPS**, not a rewrite of `/host` scans. **Coraza on Sinexis edge / full CRS = not this product.**

...
| Item | State |
|------|--------|
| **`main` tip** | Re-`git pull`. Expect **`bf32d632`** (`feat: invoice HTML print (SPA)` **#786**) or newer. Print S1a **#785**. Do **not** SSH Alembic after a green `main` deploy. |
| **This session** | Owner said **`buat`** on Inbox “Delivered”. **S0** = [`docs/specs/inbox-delivered-v1.md`](docs/specs/inbox-delivered-v1.md) (docs-only). Chat user = **product**. Print S1a/S1b **shipped**. Invoice v1 **pay loop closed**. **Never** put real account numbers / holder names / emails in git. |
...
| **Engineering default** | **§1.3.1:** Uptime advanced + **P15 S1–S9** + print S1a/S1b **shipped**. Inbox S0 this branch — next code is named **S1** (schema+API) or **S1b** (SPA `/inbox`). Copy = Sent/Failed, **not** mailbox DSN. **No** Guard Discover / invoice SMTP / bounce. Invoice v1 closed. **Do not** re-implement P12/P13 or P15 chrome. **Do not** start WAF **1147+**, P14 **G/H** unless named + `buat`. Speak **Bahasa Indonesia**. Prefix git **`GIT_MASTER=1`**. Never work on `main`. Never print tokens/IPs/customer paths/bank details. Never commit `.omo/` or screenshot PNGs. |
...
3. If user says **“lanjut”** without a named slice: recommend **S1** then **S1b** in [`docs/specs/inbox-delivered-v1.md`](docs/specs/inbox-delivered-v1.md); wait for named + `buat`. Print S1a/S1b + P15 S1–S9 + Uptime advanced are **shipped**. Invoice v1 pay loop is **closed**. **Do not** pitch GTM/AM/10 SIDs. **Do not** silent-code Guard Discover / bounce / invoice SMTP / WeasyPrint. Do **not** re-open bank env CI / compose `INVOICE_BANK_*` unless deploy is red or bank copy is dashes again.

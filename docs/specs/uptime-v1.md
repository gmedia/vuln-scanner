...
**Status:** **S1–S5 + v1 gaps + retention on `main`** (#397–#401) — Q1–Q8 locked (include seats, 60s, RFC1918 off unless `UPTIME_ALLOW_PRIVATE`, HTTP+TCP, email only, no status page, flag on, epic P8). Purge 7d/90d, beat `uptime.purge`, `worker_uptime` on REMOTE_DATA CI deploy. **Check types beyond GET+TCP:** [`uptime-v2-check-types.md`](uptime-v2-check-types.md) (on `main` #451). **SPA timeout / expect_status / edit:** **shipped** [`uptime-advanced-settings.md`](uptime-advanced-settings.md) (sheet Advanced + PATCH clear). Do not re-implement.

...
| Item | State |
|------|--------|
| **`main` tip** | Re-`git pull`. Expect **`5e2e31a9`** (`feat/uptime overview review polish` **#808**) or newer. Do **not** SSH Alembic after a green `main` deploy. Alembic head: `merge_invite_kind_guard_disabled`. |
| **This session** | Chat user = **product**. Asked to update related `.md` so AM *may later* use “ganti Imunify360 dengan Sinexis”. **Docs only** — canonical [`docs/specs/vps-displace-imunify-dev-plan.md`](docs/specs/vps-displace-imunify-dev-plan.md) **§7**. **Sentence-ok stays OPEN.** Short line still **forbidden**. H9 / Host `invoicable` **not** named. **Never** put real account numbers / holder names / emails / SIDs in git. |
...
| **Sentence-ok** | **OPEN.** Short “ganti Imunify360 dengan Sinexis” **forbidden** until Demo-ok `tc5` **AND** Invoice-ok **C1** (GMD `service_id`, off-repo) **AND** pack 1005+ (shipped) **AND** Simulate lab-gate (shipped) **AND** ModSec on lab vhost **AND** product owner dates private checklist. **AND not OR.** Do **not** treat pack-in-git as GREEN. Spec [`vps-displace-imunify-dev-plan.md`](docs/specs/vps-displace-imunify-dev-plan.md) §7. |
...
8. Sentence-ok (only if asked): long pitch only ([`vps-displace-imunify-dev-plan.md`](docs/specs/vps-displace-imunify-dev-plan.md) §0). Short “ganti Imunify…” **forbidden** until owner dates §7 E. Demo-ok = `tc5` only. `--apply-vhost` needs **named** vhost allow (Guard standing permission ≠ vhost).
9. Older snapshots (including **2026-09-17 first-scan/D10**, **2026-09-16 print + inbox**, **2026-09-15 Billing empty**, **invoice tests / metering v3**, **SPA chrome**, **#733 / tc5**): [`docs/archive/handoff-session-snapshots-2026-09.md`](docs/archive/handoff-session-snapshots-2026-09.md). Stuck-job notes: [`docs/archive/handoff-scan-pending-2026.md`](docs/archive/handoff-scan-pending-2026.md).

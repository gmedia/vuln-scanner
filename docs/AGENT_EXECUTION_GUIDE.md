...
**Repo tip at write time:** re-`git pull` after reset. **P12 Host Protect S0–S6 shipped**; **S7–S12 spek** (honest on-box; no app code until `implement`). **P13 Host WAF S0–S5**. Prod compose: `HOST_WAF_ENABLED` default **true** (control plane / SPA tab). Site WAF mode still **off** until admin sets detect. Never paste WAF onto `sinexis.app` edge nginx. Open residual: **GTM human**; Host Protect **IDR/`service_id`**; **S7 mock honesty** not yet coded.

...
**Last updated:** 2026-09-07 (VPS displace-lite **DL0–DL2** + Demo/Invoice/Sentence-ok in [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md); P14 waves still in `imunify-class-onbox.md` §7)
...
| **P14** | **Imunify-class on-box** | Owner lock: take **regional** Imunify-shaped budget (VPS/colo **without** ripping Imunify on cPanel farms) | Helper-first scan; worker never impersonates customer FS; isolate on VM; WAF on **customer** nginx; spek [`imunify-class-onbox.md`](specs/imunify-class-onbox.md); **VPS displace-lite plan** [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) | Clone / “Imunify parity” PRs; trademarks; PHP PD/KernelCare v1; SaaS SSH; edge WAF; “ganti Imunify” on **shared cPanel** | **Docs + P12 S11/S12 landed.** Slices **A–H**. **Waves 0–3** in spek §7. Displace **DL0–DL2** (not P14 **D**) only if named + `buat`. Short “ganti Imunify” only after **Sentence-ok**. Default next: **DL0**, not G/H. Legal: [`imunify-beside-not-roadmap.md`](commercial/imunify-beside-not-roadmap.md). |

...
| **P13** | **Host WAF** (on-box HTTP filter) | Second Imunify-like attach: request block/detect | Policy per `host_sites`; mock S1–S3; Coraza/nginx **on customer VPS** S4+; spek [`host-waf-v1.md`](specs/host-waf-v1.md) | Coraza on **Sinexis** edge; Imunify rules; PHP PD; ERP lab | **S0–S5 + SPA copy on `main`** (#512–#517). Flag `HOST_WAF_ENABLED` default **false**. Detect-only lab; do **not** sell protect. Do **not** put WAF on `sinexis.app` nginx. Imunify = beside, not roadmap: [`imunify-beside-not-roadmap.md`](commercial/imunify-beside-not-roadmap.md). |

...
4. **Tier / pack limits** (policy + soft product checks): Basic 1 · Pro ≤3 · Multi ≤10 **enabled** schedule-or-asset slots (align with existing **10 enabled schedules / org** cap — do not invent a second hard ceiling without product lock).
...
| Cap | Reuse **MAX_SCHEDULES_PER_ORG = 10** for enabled schedules; product copy for Basic/Pro/Multi is commercial — optional soft warning in UI when over tier, not a second silent hard-fail without lock |
...
| `POST` | `/api/assets/{id}/schedules` | create schedule prefilled from asset |
...
|-------|-------------|---------|
| **S0** | This spek approved + guide checkbox | — |
| **S1** | Model + migration + API CRUD + AuthZ tests | S0 |
| **S2** | `asset_id` on schedules + create-from-asset | S1 |
| **S3** | SPA list/form + org switch cache keys | S1 |
| **S4** | Soft tier hints + multi-target pack export hook | S2–S3, commercial |
| **S5** | Docs/ops + edge smoke notes | S1–S4 |
...
- [ ] Org can CRUD named assets scoped to active org only (IDOR green)
- [ ] Schedule can reference asset; legacy schedules without asset still work
- [ ] SPA lists assets per org; switch org does not show other org’s assets
- [ ] No CMDB/Guard merge; no secrets/IPs of real customers in git
- [ ] Tier copy aligns with SKU Basic/Pro/Multi; hard cap remains schedule org cap unless product re-locks

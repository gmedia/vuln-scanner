...
**Status:** **S0** (this file). Product **P12 Host Protect S0–S6** is on `main` (malware files). This epic is **HTTP request filtering** for the **same Guard-enrolled VPS**, not a rewrite of `/host` scans.
...
| **S5** | Live lab smoke (disposable vhost) | never ERP |

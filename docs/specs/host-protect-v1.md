...
| **D2** | Positioning | **Attach beside** Imunify where panel already has it; **primary / displace-lite** on GMD VPS/colo **without** a panel suite — after [`vps-displace-imunify-dev-plan.md`](vps-displace-imunify-dev-plan.md) **Sentence-ok GREEN** 2026-09-21 (§7; nginx dedicated / no panel; not pack-in-git). **Not** “rip Imunify on cPanel shared.” |
...
| **S10** On-box YARA | Helper `packaging/host-protect-helper/sinexis_host_scan.py` + systemd oneshot/timer; Debian `Depends: wazuh-agent`; allowlist `/var/www` `/srv/www` `/home`; timeout/nice; same needles as `php_webshell.yar`; `yara` CLI **optional**. POST to S9. **Honesty #716:** `engine=yara` only after batched CLI on our pack; else `needles`. | pytest: outside jail → exit 2, no POST; needles hit dry-run. CI **without** Clam/yara packages. Lab **tc5 fixture** (not ERP). | Clam, Windows, second systemd agent |
...
| **S12** Clam optional + helper unit | Extra `engine=clam` hits **iff** `clamscan`/`clamdscan` present. systemd: `ProtectSystem=strict` + `ReadWritePaths` jail prefixes + `/var/lib/sinexis`. Token/env fail-closed (no POST). **#716:** clamd unreachable → one `clamscan`; second POST **adds** `hit_count`. Lab Clam on tc5 remains ops. | Skip if binary absent; no CVD in git; unit lists `/var/www` `/srv/www` `/home` | Required Clam in CI image |

**Honest-scan epic:** S7–S10 **on `main`**. **S11** = **#558**. **S12** = **#559**. **Y0/Y1/C1 honesty** = **#716** ([`host-protect-clam-yara-real.md`](host-protect-clam-yara-real.md)). **S5** SPA restore ≠ **S11** helper `host_commands` queue. **S8** was documentation. Do **not** re-implement. Further depth = **P14** named slices.
...
|---|--------|------------------------|
| **Q1** | List IDR (H4) / in-app Host `invoicable` (H9) | SKU file stays **working** (H4). **H9 shipped:** catalog `invoicable=true`; AM bills in **Sinexis** (`/admin/invoices`). Invoice-ok = **I1/D5**, **not** GMD Host `service_id`. |
...
v1 is successful when an org with an enrolled VPS can: **name a web root**, **run a scan**, **see a webshell hit**, **quarantine and restore**, without SSH — and AM can describe it as **Sinexis Host Protect**, not “Imunify but worse.” Full Imunify clone remains **out**. VPS displace-lite (file + thin HTTP + Scan, no PD) is the **follow-on** plan, not this epic’s DoD.

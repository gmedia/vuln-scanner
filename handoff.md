...
| **Engineering default** | **§1.3.1:** recommend **P8.x Uptime advanced**, then P15 S1, then S2 — wait for `buat`. Invoice v1 closed: **no** upgrade CTA / pay button / auto-create on signup / gateway. PDF / invoice HTML print only if named (needs S0). **Do not** re-implement P12/P13. **Do not** start WAF **1147+**, P14 **G/H**, pack-widen, or YARA/Clam unless named + `buat`. Speak **Bahasa Indonesia**. Prefix git **`GIT_MASTER=1`**. Never work on `main`. Never print tokens/IPs/customer paths/bank details. Never commit `.omo/` or screenshot PNGs. |
...
3. If user says **“lanjut”** without a named slice: recommend guide **§1.3.1** (default **P8.x Uptime advanced**); wait for `buat`. Invoice v1 pay loop is **closed**. **Do not** pitch GTM/AM/10 SIDs. **Do not** silent-code WAF/YARA/G/H or more list pages. Do **not** re-open bank env CI / compose `INVOICE_BANK_*` unless deploy is red or bank copy is dashes again.

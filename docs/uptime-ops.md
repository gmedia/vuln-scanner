...
- **Queue:** `uptime_check`. Service: `worker_uptime`. Beat: `uptime.run_due` every 15s. Create/enable also `send_task("uptime.check")` so the first probe does not wait for beat.

...
Customer VM (helper poll)
  ...
  WAF tail (separate) → POST /api/host/agent/waf-events  rule_id 1001–1149 (main)
...
|-------|------------|-------------------------------|
| **Host WAF** | ModSecurity IDs **1001–1149** on `main` (**#708** + **#825**) | **None.** Different URL, table, ID shape. |

...
| Poll once | Menu **8** / `--poll-once` **refreshes** `/usr/lib/sinexis/host-protect/sinexis_host_scan.py` from this installer (env/token unchanged), then starts the unit. First poll on a large log **does not** ingest history — cursor jumps near EOF. Do not `journalctl`. |

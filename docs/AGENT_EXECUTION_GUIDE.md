...
**Last updated:** 2026-09-03 (P14 waves 0–3 in `imunify-class-onbox.md` §7)
...
| **P14** | **Imunify-class on-box** | Owner lock: take **regional** Imunify-shaped budget (VPS/colo **without** ripping Imunify on cPanel farms) | Helper-first scan; worker never impersonates customer FS; isolate on VM; WAF on **customer** nginx; spek [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) | Clone / “Imunify parity” PRs; trademarks; PHP PD/KernelCare v1; SaaS SSH; edge WAF | **Docs + P12 S11/S12 landed.** Slices **A–H**. **Waves 0–3** in spek §7: **0** `/host` install+heartbeat (slice **C** residual) → **1** file loop → **2** WAF **F** customer nginx → **3** **G/H** research. Implement **only** named slice + `buat`. Default next: **C** / wave 0, not G. Legal: [`imunify-beside-not-roadmap.md`](commercial/imunify-beside-not-roadmap.md). |

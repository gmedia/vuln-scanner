...
**Repo tip at write time:** re-`git pull` after reset. **P12 Host Protect S0–S10 + honesty #556 on `main`**. **S11** quarantine queue = **PR #558** until squash. **S12** Clam optional + systemd `ReadWritePaths` jail. **P13 Host WAF S0–S5**. Prod compose: `HOST_PROTECT_ENABLED` and `HOST_WAF_ENABLED` default **true**. Local/CI flags stay **false**. Site WAF mode still **off** until admin sets detect. Missing worker roots must **fail** (`unreachable_root`), not persist mock hits — S7 honesty gate **#533**. Never paste WAF onto `sinexis.app` edge nginx. Open residual: **GTM human**; Host Protect **invoice lock** (`service_id`); HPP `hostscan` seed **0** — fill in `/admin/hpp`. Working list IDR in `docs/commercial/sku-host-protect.md` (not finance lock). **P14** Imunify-**class** on-box (jobs, original stack) — docs [`imunify-class-onbox.md`](specs/imunify-class-onbox.md); **do not** implement slices until named + `buat`.
...
| **P12** | **Host Protect** (on-box web malware) | Attach for VPS/colo that want “Imunify-like” **without** cloning Imunify | Named web paths on Guard agent; YARA/Clam mock+real; hits; opt-in quarantine; SPA `/host`; spek [`host-protect-v1.md`](specs/host-protect-v1.md) **§10.1 S7–S12** | PHP PD, KernelCare, **WAF (now P13)**, cPanel plugin, shared-UID farm, Imunify IP, dual `sinexis-scan` daemon | **S0–S6 on `main`**. **S7–S12 docs** (POST JSON, add-on Depends wazuh-agent, fail-closed mock). Lab yara **today**: worker fixture. **S10:** on-box helper under `packaging/host-protect-helper/`. Lab fixture on **tc5**, not ERP. SKU **IDR unset**. Do **not** re-implement S1–S9. |
| **P13** | **Host WAF** (on-box HTTP filter) | Second Imunify-like attach: request block/detect | Policy per `host_sites`; mock S1–S3; Coraza/nginx **on customer VPS** S4+; spek [`host-waf-v1.md`](specs/host-waf-v1.md) | Coraza on **Sinexis** edge; Imunify rules; PHP PD; ERP lab | **S0–S5 + SPA copy on `main`** (#512–#517). Flag `HOST_WAF_ENABLED` default **false**. Detect-only lab; do **not** sell protect. Do **not** put WAF on `sinexis.app` nginx. Clone PRs still forbidden: [`imunify-beside-not-roadmap.md`](commercial/imunify-beside-not-roadmap.md). **Job roadmap (on-box + regional):** [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) **P14**. |
| **P14** | **Imunify-class on-box** | Take **regional** Imunify-shaped budget (VPS/colo without panel suite) | Honest helper-first scan; worker never walks customer disks; quarantine on VM; WAF on **customer** nginx | Clone Imunify; trademarks; PHP PD/KernelCare v1; SaaS SSH | **Docs 2026-09-01.** Slices **A–H** in spek. Implement **only** when user names a slice + `buat`. Default next eng: **A → B → C**. |
...
| [`docs/specs/host-protect-v1.md`](specs/host-protect-v1.md) | **S0–S6 shipped**; **S7–S12 spek** | P12; SKU IDR unset; lab [`scripts/host-protect-lab-smoke.sh`](../scripts/host-protect-lab-smoke.sh); honest-scan = POST JSON + add-on, not mock-as-prod |

...
- **Owner (2026-09-15; restated 2026-09-21):** chat user is **product**. Agent = **development only** — never pitch sales / AM / marketing / GTM / first meeting. After reset, recommend named queue in guide **§1.3.1**: **Inbox S2** → **inotify** → **WAF 1147+**. **Print S2 shipped.** **H9 Host invoicable shipped.** Still wait for named + `buat`. **Do not** pitch GTM / wave-1 email / 10 SIDs as next work.

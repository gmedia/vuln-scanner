...
**Repo tip at write time:** re-`git pull` after reset. **P12 Host Protect S0–S6** + **P13 Host WAF S0–S5**. Prod compose: `HOST_PROTECT_ENABLED` and `HOST_WAF_ENABLED` default **true** (control plane / SPA). Local/CI flags stay **false**. Site WAF mode still **off** until admin sets detect. Host Protect scan is still **mock** unless `root_path` exists on the worker. Never paste WAF onto `sinexis.app` edge nginx. Open residual: **GTM human**; Host Protect **IDR/`service_id`**.
...
| **P12** | **Host Protect** (on-box web malware) | Attach for VPS/colo that want “Imunify-like” **without** cloning Imunify | Named web paths on Guard agent; YARA/Clam mock+real; hits; opt-in quarantine; SPA `/host`; spek [`host-protect-v1.md`](specs/host-protect-v1.md) | PHP PD, KernelCare, **WAF (now P13)**, cPanel plugin, shared-UID farm, Imunify IP | **S0–S6 on `main`** (#501–#511). Lab yara: bind `/var/www/host-protect-fixture` on **worker host** (`worker_ip`), not `tc5`/ERP. Flag default **off** in git. SKU **IDR unset**. Do **not** re-implement S1–S6. |

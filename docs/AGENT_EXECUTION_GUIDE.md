...
**Last updated:** 2026-09-21 (**Owner lock:** agent = **development only** — never pitch sales / AM / marketing / GTM / first meeting / 10 SIDs. Named queue **§1.3.1:** **Inbox S2** → **inotify** → **WAF 1147+**. **Print S2 shipped.** **H9 shipped.** Still wait for named + `buat`; this lock does **not** implement those slices. **Sentence-ok GREEN** — short “ganti Imunify360 dengan Sinexis” **allowed** on **nginx dedicated / no panel only**; still **forbidden** on shared cPanel. Closure [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) **§7**. **Invoice-ok = Sinexis in-app** (I1/D5), **not** GMD `service_id`. **Demo-ok `tc5` proven.** **H9 Host invoicable shipped.** Do **not** re-implement chrome / Host Protect S1–S12. Uptime overview **#807/#808**. First-scan notify **#801**; invite SMTP **#800**; Guard D10 **#802**; schedules print **#799**; findings filter **#797** + pager **#803**; status copy **#795** + incidents **#798/#804**; Alembic merge **#805**. Print S1a/S1b **#785/#786**. Inbox S0–S1c **#787–#789/#793**. P14 E hourly **already on `main`**. Invoice v1 pay loop closed **#775**.)
**Repo tip at write time:** re-`git pull` after reset — **do not pin a SHA**. **P12 Host Protect S0–S12 on `main`**: honesty **#556**, quarantine queue **#558**, helper systemd jail **#559**, **YARA/Clam honesty #716**. **Do not** re-implement S1–S12. Missing on-box root: code status **`pending_agent`** (queued, 0 hits) until helper ingest, or **`failed`/`unreachable_root`** if abandoned — **never** mock hits on public origin (**#533**). **P13 Host WAF S0–S5** plus live ingest + **starter rules only** (**#640**/#651/#708/**#825**: product IDs 1001-1164; drop Imunify/CRS/static noise on ingest and GET). **#636** audit cursor vs log; **#633** snippet in every `server{}`. Compose: `HOST_PROTECT_ENABLED` / `HOST_WAF_ENABLED` prod **true**, local/CI **false**; **`HOST_PROTECT_ALLOW_LOCAL_WALK` default false** (P14 **B**). per-site WAF still **off** until admin sets detect/protect. `SIEM_ENABLED` **git prod compose default false** (ops secret may enable). Never paste WAF onto `sinexis.app` edge nginx. **Owner (2026-09-15):** chat user is **product**, not AM. **Do not** recommend wave-1 email / 10 SIDs / finance `service_id` as the session’s next step. GTM checklists stay in `docs/commercial/` for humans who **ask**. Residual off-repo (not agent default): fill `/admin/hpp` `hostscan`. **Do not** recommend GMD Host `service_id` as next work (Invoice-ok is Sinexis in-app). H9 Host invoicable **shipped** — catalog `invoicable=true`. Residual queue starts at Inbox S2. Print S2 **shipped**. SPA Simulate `mock.sqli.1` is **lab-gated (DL0 shipped #647)** — not a new epic unless named + `buat`. Working list IDR in `docs/commercial/sku-host-protect.md` (**not** finance lock). **P14** = owner track: **Imunify360 jobs** (on-box files + HTTP), original stack — [`imunify-class-onbox.md`](specs/imunify-class-onbox.md). **Not** a clone. AM “ganti Imunify di **VPS**” = displace-lite after **Sentence-ok** — **GREEN 2026-09-21** (plan [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) **DL0–DL3 shipped**, closure **§7**; nginx dedicated / no panel only); **never** shared cPanel. Caddy out. **A–F** on this stream: **F** WAF protect = Host Multi SKU + customer nginx snippet (`SecRuleEngine On`); **never** Sinexis edge. Wave **0** installer: **#642** one-file from **sinexis.app**. **G/H** only with `buat`. Clone PRs forbidden: [`imunify-beside-not-roadmap.md`](commercial/imunify-beside-not-roadmap.md).
...
**Priority rule:** If `handoff.md`, the archive, `docs/commercial/*`, or any old session note **disagrees with this guide** on *what to build next*, **this guide wins** (**§1.3.1**), unless the user explicitly scopes a stuck-job/ops incident. **Do not** treat “user might want GTM” as a reason to pitch AM.

**Do not implement** until the user uses an explicit verb (`implement` / `buat` / `kerjakan` / `fix` / …) **or** points at an approved spec section. **Do not** answer “lanjut” with a sales / AM / marketing checklist. Point at **§1.3.1** named queue (row 1 = Inbox S2).
...
**Owner lock (2026-09-15; restated 2026-09-21):** the chat user is **product owner**. Agent work is **development only**. Sales / AM / marketing / GTM / first customer meeting / 10 SIDs are **not** the agent queue, **not** “lanjut”, **not** step 0. **Do not** open with wave-1 email, finance `service_id`, or “GTM still open so don’t code.” GTM docs remain in `docs/commercial/` for humans who **ask**; never volunteer them. Acceptance of the product in the market is **out of agent scope**.
...
5. Other code: **§1.3.1 named queue**, then **bugfix**, docs/ops hygiene, Dependabot **only if CI green + user names the PR** (do not mass-merge). **Do not** re-implement P2/P3 S1–S5, Wave B, Guard S1–S5/Http, Host Protect S1–S12, P15 chrome. WAF **1147+** is queue **#4** (named + `buat` only). P14 **G/H** stay research unless named.
...
### 1.3.1 Development queue (agent “what next” — 2026-09-21)

P0–P14 control planes are **on `main`**. Agent work is **development only** — never sales / AM / marketing / GTM. Next engineering is **depth**, not a new epic letter, unless the user names one. **Do not implement** until the slice is **named** + `buat` / `kerjakan` / `implement`. **Sentence-ok is GREEN** 2026-09-21 (displace §7; nginx dedicated / no panel only; still **forbidden** on shared cPanel) — **not** a default code slice and **not** GMD Host SIDs. **H9 Host invoicable shipped** (named `buat`; GREEN did not auto-flip). Do **not** pitch `service_id` / 10 SIDs / first AM meeting as next **code** or as the Invoice-ok gate.

**Named order (value; still wait for named + `buat`):** **1 Inbox S2** → **2 inotify** → **3 WAF 1147+**. **Print S2 shipped.** **H9 Host invoicable shipped.** Do **not** skip ahead. Do **not** start #2–#3 while a higher row is still parked unless the user names that later slice.
...
| **— shipped** | **Scan PDF / invoice HTML print** | Browser print + invoice SPA print + **S2** `format=pdf` WeasyPrint | [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) S0 + **S1a #785** + **S1b #786** + **S2 this PR** | Do **not** re-implement; invoice PDF I10 shipped 834 |
| **— shipped** | **Inbox “Delivered” (user-side)** | SMTP Sent/Failed log + SPA `/inbox` + **S1c scan_diff job link** | [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) S0 **#787** + S1 **#788** + S1b **#789** + **S1c #793** | Do **not** re-implement; bounce/SES = queue **#1**; no Guard Discover |
| **— shipped** | **P14 E** denser Host cadence | `host_sites.scan_interval` daily\|hourly; Beat org cap 2 | [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) slice **E** (Alembic `add_host_site_scan_interval`) | Do **not** re-implement hourly; inotify = queue **#2**; WAF **1147+** = **#3**; **not** 24×7 YARA on `/`; **G/H** research |
| **— shipped** | **H9 Host `invoicable`** | Host catalog `invoicable=true`; admin creates Host invoices in-app; paid Host does not set `org.sku`; unique period per product | [`sinexis-invoice-v1.md`](specs/sinexis-invoice-v1.md) I2 · [`sku-host-protect.md`](commercial/sku-host-protect.md) H9 | Finance H4 lock; silent-bundle into VPS/Scan; GMD `service_id` as gate |
...
| **— shipped** | **Schedules last-run Print** | `printFile(last_job_id, "executive")` beside Download | **#799** | Do **not** re-implement; Schedules still HTML print (ScanDetail has S2 PDF) |
...
| **— shipped** | **Print S2** (`format=pdf` / WeasyPrint) | Server-side `.pdf` of **executive** HTML. SPA `export-pdf`. Missing Pango → 503. | [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) S2 | Invoice PDF library; Celery PDF; Playwright-as-prod-PDF |
| **1 next** | **Inbox S2** (bounce / SES) | Mailbox DSN vs SMTP Sent/Failed. S0–S1c already shipped. | [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) S2 | Guard Discover; invoice SMTP; unmask recipients |
| **2** | **inotify** (on-write Host scan) | P14 E “on-write later”. Out: 24×7 YARA on `/`. | [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) slice **E** residual | 24×7 root YARA on `/`; PHP PD; KernelCare |
| **3** | **WAF 1165+** own pack | Next original rule IDs after **1001–1164**. Out: CRS / IM360 commercial DB; P14 **G/H** (research). | [`host-waf-v1.md`](specs/host-waf-v1.md) · [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) | CRS/IM360 in git; paste onto `sinexis.app` edge; G/H clone |

**“lanjut” without a named slice** → recommend **row 1 (Inbox S2)**; still **wait for `buat`**. Do **not** silent-code. Do **not** re-implement shipped rows. Do **not** pitch AM/sales/GTM.

**Still parked (not in this queue):** Capacitor/PWA [`capacitor-shell-v1.md`](specs/capacitor-shell-v1.md); AI Gateway already S1–S5 flag-off; cPanel plugin; PHP PD; org wallet; nested projects; P14 G/H research only; YARA 24x7 on root out.
...
**Agent:** **do not** lead a session with this checklist. Maintain commercial docs **on request**; **do not** invent new list prices without user. Guard/Wazuh **feature** branches only after S0 spek on `main` (or same series) **and** explicit implement; keep thin scope. Development default: **§1.3.1** named queue.
...
| [`docs/specs/vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) | **P14 follow-on plan** (2026-09-07; closure **§7** **GREEN 2026-09-21**) | Speech gate: Demo-ok proven / Invoice-ok = **Sinexis in-app** (not GMD SID) / **Sentence-ok GREEN** (nginx dedicated / no panel). **DL0–DL3 shipped**. Caddy out. No PD/WebShield/clone. Short “ganti” **allowed** nginx-only; **forbidden** shared cPanel. H9 **shipped**. Residual queue starts at Inbox S2. Print S2 **shipped**. **Do not** recommend Host `service_id` as next. |
...
| [`docs/specs/scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) | **S0 + S1a #785 + S1b #786 + S2 shipped** | Browser print + WeasyPrint `format=pdf`. Invoice PDF I10 shipped 834. |
| [`docs/specs/inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) | **S0–S1c shipped** #787–#789 + **#793** job link; first-scan **#801** | User-side SMTP Sent/Failed + SPA `/inbox`. Inbox S2 (bounce/SES) = §1.3.1 **#1**. **Not** Guard Discover / invoice SMTP. |
...
|-----------|------------|
| “lanjut” / “next” without spec | Re-read **§1.3.1** + `handoff.md`; recommend **named queue row 1 = Inbox S2** (then inotify → WAF 1147+; Print S2 + H9 shipped); still **wait for `buat`**; **never** pitch GTM/AM/sales/marketing/10 SIDs/`service_id`/first meeting; **no** silent feature coding |
...
| “buat” scan PDF / invoice print / S1a / S1b | **Shipped** #785/#786 — [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md). Clarify bug / residual only. Print S2 shipped. Invoice PDF I10 shipped 834 |
| “buat” inbox / Delivered / S1 inbox | **Shipped** #787–#789 + S1c **#793** — [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md). Clarify bug / residual only. **Inbox S2** (bounce/SES) = §1.3.1 **#1**. **Not** Guard Discover / invoice SMTP |
| “buat P14 E” / denser Host cadence | **Hourly shipped** — `host_sites.scan_interval` daily\|hourly. Clarify bug / residual only. **inotify** = §1.3.1 **#2**. **WAF 1147+** = **#3**. **Not** 24×7 YARA on `/` / **G/H** |
| “buat H9” / Host invoicable | **Shipped.** Host catalog `invoicable=true`. Spec [`sinexis-invoice-v1.md`](specs/sinexis-invoice-v1.md) I2. **Not** GMD SID / H4 finance lock. Do **not** re-implement |
| “buat Print S2” / `format=pdf` / WeasyPrint | **Shipped** this PR. Invoice PDF I10 shipped 834. Do **not** re-implement |
| “buat Inbox S2” / bounce / SES | §1.3.1 **#1**. After Print S2 (shipped) unless user names this slice |
| “buat inotify” / on-write Host | §1.3.1 **#2**. Spec P14 E residual. **Not** 24×7 YARA on `/` |
| “buat WAF 1147” / pack-widen | §1.3.1 **#3**. Own original IDs. **Not** CRS/IM360 / G/H / `sinexis.app` edge |
...
| “buat schedules print” | **Shipped #799**. Executive `printFile` only. ScanDetail PDF is Print S2 (shipped); Schedules stays HTML print |
| “GTM” / “email AM” / “10 SID” / “meeting AM” | Point to [`gtm-and-p11-ops.md`](commercial/gtm-and-p11-ops.md) **only if asked**; **never** treat as engineering next or as “lanjut” |
| “ganti Imunify” / Sentence-ok / displace-lite | Point to [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) **§0 + §7**. **Sentence-ok GREEN** 2026-09-21 — short line **allowed** on **nginx dedicated / no panel**; still **forbidden** on shared cPanel. Invoice-ok = **Sinexis in-app** (I1/D5), **not** GMD Host `service_id`. Do **not** tell the user the next step is three SIDs. Do **not** ship WAF **1147+** / **G/H** unless named. H9 Host invoicable is **shipped**. Lab Demo-ok = `tc5` only; `--apply-vhost` needs **named** vhost allow (Guard standing permission ≠ vhost) |

**Locked answers (do not re-ask every session):** chat user = **product owner** (2026-09-15; restated 2026-09-21) — agent = **development only**, default **§1.3.1** named queue, **never** GTM/AM/sales. KPI commercial = **attach ARPU** (AM owns renew; **not** an agent task). Billing = **Sinexis invoice v1** (bank transfer, mark paid) + optional GMD `service_id`. Dual-brand = **6–12 mo soft**. Guard thin + SIEM flag-off unless ops. Invoice pay loop **closed**. Metering v3: Scan seats, `credit_cost = 0`.

**Still open (human, off-repo — never lead with these):** concrete 10 SIDs, pilot #1 identity, optional Scan GTM `service_id` (**not** Invoice-ok, **not** agent next), live quote ± on IDR, `/admin/hpp` `hostscan`, first AM meeting on a customer nginx VPS. Remind **only if asked**.
...
| `handoff.md` | Session snapshot — **epic order** still this guide (**§1.3.1** development queue, never GTM) |
...
| `docs/specs/scan-pdf-invoice-print-v1.md` | **S0 + S1a #785 + S1b #786 + S2 shipped.** Browser print plus WeasyPrint PDF. Invoice PDF I10 shipped 834 |
| `docs/specs/inbox-delivered-v1.md` | **S0–S1c shipped #787–#789/#793.** User SMTP Sent/Failed + SPA `/inbox`. Bounce/SES = §1.3.1 **#1**. **Not** Guard Discover / invoice SMTP |
...
> After reset: **boot §0 → §1.3.1 named queue (Inbox S2 → inotify → WAF 1147+; Print S2 + H9 shipped; still wait for `buat`) → never pitch AM/sales/GTM → Sentence-ok GREEN 2026-09-21 (displace §7; short “ganti Imunify” **allowed** nginx dedicated / no panel; **forbidden** shared cPanel; Invoice-ok = Sinexis in-app, **not** GMD SID) → Guard live lab standing-permitted (wipe `tc5` first §4.1; do not re-ask) → no silent epics → no PII/SSH/IPs in git → Indonesian with user, `GIT_MASTER=1`.**

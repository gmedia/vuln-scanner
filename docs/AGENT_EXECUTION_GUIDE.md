...
P0–P14 control planes are **on `main`**. Next engineering is **depth**, not a new epic letter, unless the user names one. **Do not implement** until the slice is **named** + `buat` / `kerjakan` / `implement`. **Sentence-ok is OPEN** (displace §7) — Demo-ok proven; remaining = **owner-dated §7 E**, **not** a default code slice and **not** GMD Host SIDs. Do **not** pitch `service_id` / 10 SIDs as next **code** or as the Invoice-ok gate.
...
| [`docs/specs/vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) | **P14 follow-on plan** (2026-09-07; closure **§7** **2026-09-20**) | Speech gate: Demo-ok proven / Invoice-ok = **Sinexis in-app** (not GMD SID) / **Sentence-ok OPEN** (owner date). **DL0–DL3 shipped**. Caddy out. No PD/WebShield/clone. Short “ganti” still **forbidden**. **Do not** recommend Host `service_id` as next. |
...
| “ganti Imunify” / Sentence-ok / displace-lite | Point to [`vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) **§0 + §7**. **Sentence-ok OPEN** — short line **forbidden**. Invoice-ok = **Sinexis in-app** (I1/D5), **not** GMD Host `service_id`. Do **not** tell the user the next step is three SIDs. Do **not** ship WAF **1147+** / **G/H** / in-app Host `invoicable` unless named. Lab Demo-ok = `tc5` only; `--apply-vhost` needs **named** vhost allow (Guard standing permission ≠ vhost) |

...
**Do not implement** until the user uses an explicit verb (`implement` / `buat` / `kerjakan` / `fix` / …) **or** points at an approved spec section. **Do not** answer “lanjut” with a sales / AM / marketing checklist. Point at **§1.3.1** named queue (row 1 = H9).
...
5. Other code: **§1.3.1 named queue**, then **bugfix**, docs/ops hygiene, Dependabot **only if CI green + user names the PR** (do not mass-merge). **Do not** re-implement P2/P3 S1–S5, Wave B, Guard S1–S5/Http, Host Protect S1–S12, P15 chrome. WAF **1147+** is queue **#5** (named + `buat` only). P14 **G/H** stay research unless named.
...
P0–P14 control planes are **on `main`**. Agent work is **development only** — never sales / AM / marketing / GTM. Next engineering is **depth**, not a new epic letter, unless the user names one. **Do not implement** until the slice is **named** + `buat` / `kerjakan` / `implement`. **Sentence-ok is GREEN** 2026-09-21 (displace §7; nginx dedicated / no panel only; still **forbidden** on shared cPanel) — **not** a default code slice and **not** GMD Host SIDs. GREEN does **not** auto-flip H9. Do **not** pitch `service_id` / 10 SIDs / first AM meeting as next **code** or as the Invoice-ok gate.

**Named order (value; still wait for named + `buat`):** **1 H9** → **2 Print S2** → **3 Inbox S2** → **4 inotify** → **5 WAF 1147+**. Do **not** skip ahead. Do **not** start #2–#5 while a higher row is still parked unless the user names that later slice.
...
| **— shipped** | **Scan PDF / invoice HTML print** | Browser print + invoice SPA print | [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) S0 + **S1a #785** + **S1b #786** | Do **not** re-implement; `format=pdf` / WeasyPrint = queue **#2** |
| **— shipped** | **Inbox “Delivered” (user-side)** | SMTP Sent/Failed log + SPA `/inbox` + **S1c scan_diff job link** | [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) S0 **#787** + S1 **#788** + S1b **#789** + **S1c #793** | Do **not** re-implement; bounce/SES = queue **#3**; no Guard Discover |
| **— shipped** | **P14 E** denser Host cadence | `host_sites.scan_interval` daily\|hourly; Beat org cap 2 | [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) slice **E** (Alembic `add_host_site_scan_interval`) | Do **not** re-implement hourly; inotify = queue **#4**; WAF **1147+** = **#5**; **not** 24×7 YARA on `/`; **G/H** research |
...
| **1 next** | **H9 Host `invoicable`** | Flip Host catalog `invoicable` so Sinexis can invoice Host SKU in-app (I2 leftover). **Not** GMD Host SID. GREEN does **not** auto-flip. | [`sinexis-invoice-v1.md`](specs/sinexis-invoice-v1.md) I2 · [`sku-host-protect.md`](commercial/sku-host-protect.md) H9 | Finance H4 lock; silent-bundle into VPS/Scan; GMD `service_id` as gate |
| **2** | **Print S2** (`format=pdf` / WeasyPrint) | Server-side `.pdf` bytes for scan export. Today `format=pdf` **400**. Out: invoice PDF (I10 stays out). | [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) S2 | Invoice PDF library; Celery PDF; Playwright-as-prod-PDF |
| **3** | **Inbox S2** (bounce / SES) | Mailbox DSN vs SMTP Sent/Failed. S0–S1c already shipped. | [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) S2 | Guard Discover; invoice SMTP; unmask recipients |
| **4** | **inotify** (on-write Host scan) | P14 E “on-write later”. Out: 24×7 YARA on `/`. | [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) slice **E** residual | 24×7 root YARA on `/`; PHP PD; KernelCare |
| **5** | **WAF 1147+** own pack | Next original rule IDs after **1001–1146**. Out: CRS / IM360 commercial DB; P14 **G/H** (research). | [`host-waf-v1.md`](specs/host-waf-v1.md) · [`imunify-class-onbox.md`](specs/imunify-class-onbox.md) | CRS/IM360 in git; paste onto `sinexis.app` edge; G/H clone |

**“lanjut” without a named slice** → recommend **row 1 (H9)**; still **wait for `buat`**. Do **not** silent-code. Do **not** re-implement shipped rows. Do **not** pitch AM/sales/GTM.
...
| [`docs/specs/vps-displace-imunify-dev-plan.md`](specs/vps-displace-imunify-dev-plan.md) | **P14 follow-on plan** (2026-09-07; closure **§7** **GREEN 2026-09-21**) | Speech gate: Demo-ok proven / Invoice-ok = **Sinexis in-app** (not GMD SID) / **Sentence-ok GREEN** (nginx dedicated / no panel). **DL0–DL3 shipped**. Caddy out. No PD/WebShield/clone. Short “ganti” **allowed** nginx-only; **forbidden** shared cPanel. H9 = §1.3.1 **#1** (named, wait `buat`). **Do not** recommend Host `service_id` as next. |
...
| [`docs/specs/scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md) | **S0 + S1a #785 + S1b #786 shipped** | Browser print. Print S2 (`format=pdf` / WeasyPrint) = §1.3.1 **#2**. |
| [`docs/specs/inbox-delivered-v1.md`](specs/inbox-delivered-v1.md) | **S0–S1c shipped** #787–#789 + **#793** job link; first-scan **#801** | User-side SMTP Sent/Failed + SPA `/inbox`. Inbox S2 (bounce/SES) = §1.3.1 **#3**. **Not** Guard Discover / invoice SMTP. |
...
|-----------|------------|
| “lanjut” / “next” without spec | Re-read **§1.3.1** + `handoff.md`; recommend **named queue row 1 = H9** (then Print S2 → Inbox S2 → inotify → WAF 1147+); still **wait for `buat`**; **never** pitch GTM/AM/sales/marketing/10 SIDs/`service_id`/first meeting; **no** silent feature coding |
...
| “buat” scan PDF / invoice print / S1a / S1b | **Shipped** #785/#786 — [`scan-pdf-invoice-print-v1.md`](specs/scan-pdf-invoice-print-v1.md). Clarify bug / residual only. **Print S2** (`format=pdf` / WeasyPrint) = §1.3.1 **#2** — only if named after H9 |
| “buat” inbox / Delivered / S1 inbox | **Shipped** #787–#789 + S1c **#793** — [`inbox-delivered-v1.md`](specs/inbox-delivered-v1.md). Clarify bug / residual only. **Inbox S2** (bounce/SES) = §1.3.1 **#3**. **Not** Guard Discover / invoice SMTP |
| “buat P14 E” / denser Host cadence | **Hourly shipped** — `host_sites.scan_interval` daily\|hourly. Clarify bug / residual only. **inotify** = §1.3.1 **#4**. **WAF 1147+** = **#5**. **Not** 24×7 YARA on `/` / **G/H** |
| “buat H9” / Host invoicable | §1.3.1 **#1**. Flip Host catalog `invoicable`. Spec [`sinexis-invoice-v1.md`](specs/sinexis-invoice-v1.md) I2. GREEN does **not** auto-flip. **Not** GMD SID / H4 finance lock |
| “buat Print S2” / `format=pdf` / WeasyPrint | §1.3.1 **#2**. After H9 named. Invoice PDF still I10 **out** |
| “buat Inbox S2” / bounce / SES | §1.3.1 **#3**. After Print S2 unless user names this slice |
| “buat inotify” / on-write Host | §1.3.1 **#4**. Spec P14 E residual. **Not** 24×7 YARA on `/` |
| “buat WAF 1147” / pack-widen | §1.3.1 **#5**. Own original IDs. **Not** CRS/IM360 / G/H / `sinexis.app` edge |
...
> After reset: **boot §0 → §1.3.1 named queue (H9 → Print S2 → Inbox S2 → inotify → WAF 1147+; still wait for `buat`) → never pitch AM/sales/GTM → Sentence-ok GREEN 2026-09-21 (displace §7; short “ganti Imunify” **allowed** nginx dedicated / no panel; **forbidden** shared cPanel; Invoice-ok = Sinexis in-app, **not** GMD SID; GREEN does not auto-flip H9) → Guard live lab standing-permitted (wipe `tc5` first §4.1; do not re-ask) → no silent epics → no PII/SSH/IPs in git → Indonesian with user, `GIT_MASTER=1`.**

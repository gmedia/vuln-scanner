...
**Repo tip at write time:** re-`git pull` after reset. **P12 Host Protect S0–S6** + **P13 Host WAF S0–S5**. Prod compose: `HOST_WAF_ENABLED` default **true** (control plane / SPA tab). `HOST_PROTECT_ENABLED` still **false** until ops. Site WAF mode still **off** until admin sets detect. Never paste WAF onto `sinexis.app` edge nginx. Open residual: **GTM human**; Host Protect **IDR/`service_id`**.

...
# ---------------------------------------------------------------------------
# Retry loop coverage — _send_with_retry exponential backoff
# ---------------------------------------------------------------------------
...
class TestRetryLoop:
    ...
    async def test_retry_then_succeed(self, monkeypatch):
        monkeypatch.setattr(email_module, "_MAX_RETRIES", 3)

        mock_smtp = AsyncMock()
        mock_smtp.connect = AsyncMock(side_effect=[OSError("Timeout"), None])
        mock_smtp.send_message = AsyncMock()
        mock_smtp.quit = AsyncMock()

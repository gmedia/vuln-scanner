...
**Follow-on:** protect-on-customer-nginx is **P14 slice F** / **wave 2** — [`imunify-class-onbox.md`](imunify-class-onbox.md) §7. Original pack widen = **D2** in [`vps-displace-imunify-dev-plan.md`](vps-displace-imunify-dev-plan.md). Protect mode is **Host Multi** only; snippet still never for `sinexis.app` edge.

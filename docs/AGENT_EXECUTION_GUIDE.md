...
**Repo tip at write time:** re-`git pull` after reset. **P12 Host Protect S0–S6 shipped**; **S7–S12 spek** (honest on-box). **P13 Host WAF S0–S5**. Prod compose: `HOST_PROTECT_ENABLED` and `HOST_WAF_ENABLED` default **true** (control plane / SPA). Local/CI flags stay **false**. Site WAF mode still **off** until admin sets detect. Missing worker roots must **fail** (`unreachable_root`), not persist mock hits — S7 honesty gate is **#533**. Never paste WAF onto `sinexis.app` edge nginx. Open residual: **GTM human**; Host Protect **IDR/`service_id`**; S11–S12 not coded (S9 ingest + S10 helper in flight).

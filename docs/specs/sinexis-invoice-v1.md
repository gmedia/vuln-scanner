...
| **I2** | What is sold | **Scan SKU seats** only: Basic / Pro / Multi. List IDR from P0 lock (300_000 / 650_000 / 2_000_000). Host Protect catalog rows may be seeded **working** but **not invoiced** in v1 (H9 still open). **Invoice-ok** for the short “ganti Imunify” line is **this Sinexis in-app path** (I1), **not** GMD `service_id`, **not** flipping Host `invoicable` — see [`vps-displace-imunify-dev-plan.md`](vps-displace-imunify-dev-plan.md) §7.3. |

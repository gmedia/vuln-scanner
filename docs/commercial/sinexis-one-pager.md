...
**Overage:** more named targets → upgrade tier. Scans (manual and scheduled) do **not** spend credits. **Invoice:** Sinexis `/admin/invoices` (I1/D5 preferred). Optional leftover: GMD `service_id` per tier — **not** Invoice-ok, **not** agent next. No silent VPS bundle.

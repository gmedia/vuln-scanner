# ARCHIVED / STALE — scan job “pending” investigation handoff
...
>
> **Priority rule:** If anything below disagrees with `docs/AGENT_EXECUTION_GUIDE.md` on *what to build next*, **the execution guide wins**, unless the user explicitly names an active stuck-job / worker incident.
>
> **Before any code work inspired by this archive:** re-verify against current `main` (many items below were diagnosis-only or have since shipped, e.g. `/health/queues`, auto-fail maintenance). Do not re-implement from the PENDING list blindly.
>
> **Original context date:** early stuck-pending diagnosis (pre–Sinexis roadmap). Archived 2026-08-07.
> **Stub at repo root:** [`handoff.md`](../../handoff.md) (redirect only).

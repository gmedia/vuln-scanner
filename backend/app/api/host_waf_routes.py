...
@router.get("/sites/{site_id}/snippet", response_model=HostWafSnippetResponse)
async def snippet(
    request: Request,
    site_id: UUID,
    current_user: User = Depends(get_current_user),
    ...
    return await HostWafService(db).snippet(current_user, get_active_org_id(request), site_id)
